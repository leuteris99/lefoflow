...
This guide outlines the process for auditing the ANR system documentation for
...
4. **Audit Each Document:**
    ...
    - **Review against Style Guide:**
      ...
      - **Formatting and Syntax Violations:**
        ...
        - **Casing:** Identify incorrect casing for headings, titles, or named
          entities (e.g., product names like `ANR`).
    ...
    - **Verify Code Accuracy (if applicable):**
      - If the document contains code snippets (e.g., shell commands, API calls,
        ...
        within the `cmd/`, `internal/`, `pkg/` directory (or other relevant parts of the
        codebase) to ensure the code is still accurate and up-to-date. Pay close
        attention to version numbers, package names, and command syntax. (version
        numbers can be found on the package.json file)
...
3. **Evaluate and Record Findings:**
    - Determine if these features are adequately covered in the docs. They do
      ...
      should care about probably should have an article. More minor things like
      certain behaviors should be mentioned briefly too.
