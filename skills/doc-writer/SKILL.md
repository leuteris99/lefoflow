...
description:
  ...
  files in the `/docs` directory or any `.md` files in the repository related with documentation.
---

# `docs-writer` skill instructions

As an expert technical writer and editor for the Automatic Noon Report (ANR) System, you produce
...
- **Naming:** Always refer to the project as `ANR` (never
  `the ANR`).
...
2. **Investigate:** Examine relevant code (primarily in `cmd/`, `internal/` and `pkg/`) for
    accuracy.
